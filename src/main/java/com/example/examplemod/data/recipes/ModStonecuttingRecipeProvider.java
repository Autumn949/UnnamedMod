 ...
 * stonecutting recipes from the registered recipes.
 */
public class ModStonecuttingRecipeProvider extends RecipeProvider {
    ...
    public ModStonecuttingRecipeProvider(DataGenerator gen) {
        super(gen);
    }
            ...
            for (Pair<Item, Integer> result : results) {
                ...
                if (!flushed.contains(name)) {
                    SingleItemRecipeBuilder.stonecuttingRecipe(Ingredient.fromItems(from), to, count)
                                           ...
                                           .build(consumer, UnnamedMod.id(name));
                    flushed.add(name);
                }
            }
        }
